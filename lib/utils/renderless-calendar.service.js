...
export function getPrevMonthDates(date, firstDayOfWeek = 1) {
  return _getMonthLastNDays(date.getDay(), date.getFullYear(), date.getMonth());
}
...
export function getCurrentMonthDates(year, month, firstDayOfWeek = 1) {
  ...
  return dates.filter(date => isSameMonth(month, date.getMonth()));
}

export function getNextMonthDates(date, firstDayOfWeek = 1) {
  const numberOfDays = ZERO_BASED_DAYS_IN_WEEK - date.getDay();
  return _getMonthFirstNDays(numberOfDays, date.getFullYear(), date.getMonth());
}
...
export function _getMonthLastNDays(n, year, month, firstDayOfWeek = 1) {
  ...
  while (dates.length < n) {
    ...
    if (!isSameMonth(month, date.getMonth())) {
      dates.unshift(date);
    }
...
export function _getMonthFirstNDays(n, year, month, firstDayOfWeek = 1) {
  ...
  for (let dayNumber = 1; dayNumber <= n; dayNumber++) {
    dates.push(new CalendarDate(year, nextMonthNumber, dayNumber));
  }
...
export function breaksLowerLimit(minDate, currentYear, currentMonth) {
  ...
  const date = new Date(minYear, minMonth - MONTH_INDEX_CORRECTION, minDay);
  const minDateYear = date.getFullYear();
  const minDateMonth = date.getMonth();
  const prevMonth = currentMonth - 1;

  return minDateYear === currentYear && isGreaterThan(minDateMonth, prevMonth);
}
...
export function breaksUpperLimit(maxDate, currentYear, currentMonth) {
  ...
  const date = new Date(maxYear, maxMonth - MONTH_INDEX_CORRECTION, maxDay);
  const maxDateYear = date.getFullYear();
  const maxDateMonth = date.getMonth();
  const nextMonth = currentMonth + 1;

  return maxDateYear === currentYear && isLessThan(maxDateMonth, nextMonth);
}
