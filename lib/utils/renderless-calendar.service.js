...
export function generateCalendarViewData({
  ...
  while (it < endMonth) {
    ...
    const prevMonthDates = omitOtherMonthDays ? [] : getPrevMonthDates(currentMonthDates[0], firstDayOfWeek);
    const nextMonthDates = omitOtherMonthDays ? [] : getNextMonthDates(currentMonthDates[currentMonthDates.length - 1], firstDayOfWeek);
...
export function getMonthsList({
  ...
  const lastIndex = startIndex + Math.max(viewState.length, 12);
